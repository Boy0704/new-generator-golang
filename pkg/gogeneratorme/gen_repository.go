...
const BASE_PATH = "./internal/app/repositories/"

func CreateRepository(nmString string, mystruct interface{}) string {
	createFolderRepository(nmString)
	return "s"
	//createFile(mystruct)
	//writeFile(mystruct)
...
func createFolderRepository(nmString string) error {
	path := filepath.Join(BASE_PATH, nmString)
	_, err := os.Stat(path)
